...
import {timestamp2LocalDateTimeString, timestamp2LocalDateString} from '@kaseihaku.com/core-com';
...
export function sortPojoSchemaByDisplayName(pojoSchemaAry){

  let name2DisplayNameMap = new Map();

  pojoSchemaAry.forEach(item => {
    let displayName = getDisplayName(item);
    name2DisplayNameMap.set(item.name, displayName);
  });
  ...
  return pojoSchemaAry.toSorted((a, b)=>{
    ...
    let aDisplayName = name2DisplayNameMap.get(a.name);
    let bDisplayName = name2DisplayNameMap.get(b.name);
