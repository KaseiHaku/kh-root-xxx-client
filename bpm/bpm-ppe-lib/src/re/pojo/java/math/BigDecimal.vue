<template>
  <el-input-number v-model="pojoInsCpt" :disabled="readonlyInit"></el-input-number>
</template>
...
function initialize(){
  ...
  if(maxAnno){
    max.value = Number(maxAnno.attributes.value);
  }

  let minAnno = findAnnoFirst(allProps.pojoSchema, RecognizedAnno.jsr303.Min);
  if(minAnno){
    min.value = Number(minAnno.attributes.value);
  }

  let decimalMaxAnno = findAnnoFirst(allProps.pojoSchema, RecognizedAnno.jsr303.DecimalMax);
  if(decimalMaxAnno){
    max.value = Number(decimalMaxAnno.attributes.value);
  }

  let decimalMinAnno = findAnnoFirst(allProps.pojoSchema, RecognizedAnno.jsr303.DecimalMin);
  if(decimalMinAnno){
    min.value = Number(decimalMinAnno.attributes.value);
  }
  ...
  let digitsAnno = findAnnoFirst(allProps.pojoSchema, RecognizedAnno.jsr303.Digits);
  if(digitsAnno){
    precision.value = Number(digitsAnno.attributes.fraction);
  }
