  ...
  <div>
    <el-form :inline="true" :model="searchFormData" label-width="auto" label-position="right">
      <el-row justify="start" align="top">
        <el-col :span="6">
          <el-form-item label="流程商标:" prop="metaBrand">
            <el-input v-model="searchFormData.metaBrand" placeholder="metaBrand" @keyup.enter="doSearch" :disabled="procMetaBrand"/>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="节点名称:" prop="versionName">
            <el-input v-model="searchFormData.versionName" placeholder="versionName" @keyup.enter="doSearch" />
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
    <el-table :data="tableData" stripe border>
      ...
      <el-table-column prop="metaBrand" label="流程商标" :formatter="procBrand2Name" />
      <el-table-column prop="versionName" label="流程版本名称" />
...
const searchFormData = reactive({
  metaBrand: allProps.procMetaBrand,
  versionName: '',
});
async function doSearch() {
  ...
  const body = await BpmPpeServerApi.default.bpmPpe.v1.Process.apply.put(params);
...
const handleView = ({ row, column, $index }) => {
  ...
  window.open(`/bpm-ppe/editor.html?bpmAppName=${bpmAppName}#/?procId=${row.id}`, '_blank');
};
