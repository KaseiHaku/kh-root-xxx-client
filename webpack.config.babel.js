...
export default {
  ...
  entry: () => ({
    ...
    'index': {
      ...
      import: ['./core/core-com/src/index.esm.js'],
  ...
  module: {
    ...
    rules: [
      ...
      {
        resourceQuery: /resource/,  // 匹配 import './xxx/yyy?resource';
        type: 'asset/resource',
      },
   ...
   *
   * */
  devtool: process.env.APP_PROFILE !== 'prod' ? 'source-map' : false, // prod 环境不生成 source-map, 其他环境都生成，方便 debug
  ...
  devServer: {
    ...
    hot: true,   // 开启 module 热替换, 'only'=如果构建失败，不以刷新页面作为 兜底策略
     ...
     * */
    proxy: [
      { context: ['/login', '/oauth2-login', '/logout'], target: 'http://192.168.0.106:8910' },
      { context: ['/oauth2'], target: 'http://192.168.0.106:8910', secure: false, toProxy: true },
      { context: ['/consul'], target: 'http://192.168.0.200:8500', pathRewrite: { '^/consul': '' }, secure: false, },
      {
        context: ['/api'],
